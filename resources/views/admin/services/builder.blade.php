...
<head>
    ...
    <script>
        ...
        var builder = new Editor({
            ...
            urlBack: "{{ route('admin.services.index') }}",
            backUrl: "{{ route('admin.services.index') }}",
            uploadAssetUrl: "{{ route('admin.services.uploadAsset') }}",
            uploadAssetMethod: 'POST',
            saveUrl: "{{ route('admin.services.editServiceTheme', ['id' => request()->id]) }}",
            saveMethod: 'POST',
            url: "{{ route('admin.services.getTheme', ['id' => request()->id]) }}",
            headers: {
