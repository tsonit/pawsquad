                    ...
                    <div class="col-12 col-12 order-1">
                            ...
                            <div class="card-body">
                                <div class="mb-6">
                                    ...
                                    <input type="text" class="form-control" placeholder="Tên sản phẩm" name="name"
                                        id="title" aria-label="Tên sản phẩm" value="{{ old('name',$data->name) }}"
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea class="form-control" placeholder="Mô tả ngắn sản phẩm" name="short_description" id="short_description"
                                        aria-label="Mô tả ngắn sản phẩm" rows="5">{{ old('short_description',$data->short_description) }}</textarea>
                                </div>
                                <div class="mb-6">
                                    <label class="mb-1">Mô tả</label>
                                    <textarea class="tsonit w-100" rows="4" cols="12" name="description">{!! html_decode(old('description',$data->description)) !!}</textarea>
                                </div>
                                <div class="align-items-center border-top pt-2 status">
                                    <span class="mb-0 me-3">Hiển thị</span>
                                    <select name="status" class="form-select">
                                        <option value="0" {{ old('status',$data->status) == 0 ? 'selected' : '' }}>Nháp</option>
                                        <option value="1" {{ old('status',$data->status) == 1 ? 'selected' : '' }}>Hiện</option>
                                        <option value="2" {{ old('status',$data->status) == 2 ? 'selected' : '' }}>Ẩn</option>
                                    </select>
                                </div>
                                <div class="mb-6 border-top mt-3">
                                    ...
                                    <div class="input-group">
                                        <select class="form-control featured" name="featured" id="featured">
                                            <option value="1" {{ old('featured',$data->featured) == 1 ? 'selected' : '' }}>Nổi bật
                                            </option>
                                            <option value="0" {{ old('featured',$data->featured) == 0 ? 'selected' : '' }}>Không
                                                nổi bật</option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6 mb-6">
                                        ...
                                        <select id="category" class="select2 form-select"
                                            ...
                                            @forelse($categories as $row)
                                                <option value="{{ $row->id }}"
                                                    {{ old('category', $data->category_id) == $row->id ? 'selected' : '' }}>
                                                    {{ $row->name }}
                                                </option>
                                            @empty
                                    ...
                                    <div class="col-md-6 mb-6">
                                        ...
                                        <select id="brands" class="select2 form-select"
                                            ...
                                            @forelse($brands as $row)
                                                <option value="{{ $row->id }}"
                                                    {{ old('brands',$data->brand_id) == $row->id ? 'selected' : '' }}>
                                                    {{ $row->name }}
                                                </option>
                                            @empty
                        ...
                        <div class="card mb-6">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <div class="form-check form-switch">
                                        ...
                                        <input type="checkbox" class="form-check-input" id="is_variant"
                                            onchange="isVariantProduct(this)" name="is_variant" @if ($data->has_variation) checked @endif>
                                    </div>
                                </div>

                                <div class="noVariation" @if ($data->has_variation) style="display:none;" @endif>
                                    @php
                                        $first_variation = $data->variations->first();
                                        if(!$first_variation && !$data->has_variation) {
                                            $price = $data->min_price;
                                        }else{
                                            $price = !$data->has_variation ? $first_variation->price : 0;
                                        }
                                        $stock_qty = !$data->has_variation ? ($first_variation->product_variation_stock ? $first_variation->product_variation_stock->stock_qty : 0) : 1;
                                        $code = !$data->has_variation ? $first_variation->code : null;
                                    @endphp
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" id="code" placeholder="Mã sản phẩm"
                                                    name="code" class="form-control" {{ !$data->has_variation ? 'required' : '' }} value="{{ old('code',$code) }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="hasVariation" @if (!$data->has_variation) style="display:none;" @endif>
                                    @foreach (generateVariationOptions(
                                        $data->variation_combinations()->where('variation_id','!=', 1)->get(),
                                    ) as $key => $combination)
                                        <div class="row g-3 mt-2">
                                            ...
                                            <div class="col-lg-6">
                                                <div class="variationvalues">
                                                    @php
                                                        $variation_values = \App\Models\VariationValues::where('variation_id','!=', 1)->where('variation_id', $combination['id'])
                                                            ->get();
                                                    ...
                                                    <div class="d-flex">
                                                        ...
                                                        <button type="button" data-toggle="remove-parent"
                                                            ...
                                                            onclick="generateVariationCombinations()">
                                                                <i class="me-1 menu-icon tf-icons ti ti-trash text-danger"></i>
                                                        </button>
                                                    </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="variation_combination" id="variation_combination">
                                            ...
                                            @if ($data->has_variation)
                                                @include(
                                                    'admin.partials.products.update_variation_combinations',
                                                    [
...
@section('js')
    ...
    @include('admin.partials.products.script_edit_attributes')
            ...
            function initializeDropzone(selector, multipleFiles = false) {
                ...
                return new Dropzone(selector, {
                    ...
                    init: function() {
                        ...
                        var sessionProduct = {!! json_encode(session('product')) !!};
                        if (sessionProduct) {
                            // Xử lý ảnh đại diện
                            if (sessionProduct.image) {
                                $.ajax({
                                    type: 'GET',
                            ...
                            // Xử lý ảnh chi tiết
                            if (sessionProduct.listimg) {
                                var sessionListImgString = JSON.stringify(sessionProduct.listimg);
                                var detailImages = JSON.parse(sessionListImgString);

                                if (detailImages.length > 0 && fileType === 'detail') {
                                    var totalFiles = detailImages.length;
                                    var processedFiles = 0;

                                    detailImages.forEach(function(imageArray, index) {
                                        if (imageArray.length > 0) {
                                            $.ajax({
                                                url: "{{ route('admin.file.get-file-info') }}",
                                                data: {
                                                    fileUrl: imageArray
                                                },
                                                dataType: "json",
                                                success: function(response) {
                                                    ...
                                                    dropzone.emit("addedfile",
                                                        detailFile);
                                                    dropzone.emit("thumbnail",
                                                        detailFile, fileInfo.url
                                                    );
                                                    dropzone.emit("complete",
                                                        detailFile);
                                                    // Thêm URL vào thẻ <a> của Fancybox
                                                    var thumbnailElement =
                                                        detailFile.previewElement
                                                        .querySelector(
                                                            '[data-fancybox]');
                                                    if (thumbnailElement) {
